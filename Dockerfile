...
RUN mkdir -p /tmp/stow-test && echo "=== Stow dry-run ===" && for pkg in packages/*/; do name=$(basename "$pkg"); stow --simulate --target=/tmp/stow-test --dir=packages "$name" || exit 1; echo "  $name: OK"; done && echo "ALL PASSED"
